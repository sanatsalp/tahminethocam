...
BEGIN
  ...
  SELECT
    COALESCE(tower_game_enabled, true),
    COALESCE(tower_game_visible, true),
    COALESCE(tower_game_maintenance, false),
    COALESCE(tower_game_max_bet_amount, 50),
    COALESCE(tower_game_daily_play_limit, 3)
  INTO
  ...
  SELECT games_played INTO v_games_played
  FROM tower_game_limits
  ...
  UPDATE tower_game_limits
  SET games_played = games_played + 1
  WHERE user_id = v_user_id AND date = v_today;

  -- Deduct bet (atomic guard).
  UPDATE profiles
  SET credits = credits - p_bet_amount
  WHERE id = v_user_id AND credits >= p_bet_amount;
...
BEGIN
  ...
  IF p_chosen_index = v_safe_index THEN
    v_next_multiplier := LEAST(v_multiplier_cap, POWER(v_multiplier_step, v_next_level));
    is_correct := true;
  ...
  ELSE
    ...
    next_multiplier := LEAST(v_multiplier_cap, POWER(v_multiplier_step, p_current_level)); -- keeps multiplier at last safe level
    RETURN NEXT;
  END IF;
END;
$$;
...
BEGIN
  ...
  v_multiplier := LEAST(v_multiplier_cap, POWER(v_multiplier_step, v_correct_count));

  UPDATE tower_game_sessions
...
BEGIN
  ...
  FOR v_i IN 1..array_length(p_picked_indices, 1) LOOP
    ...
    ELSE
      ...
      final_level := v_correct_count;
      final_multiplier := v_multiplier;
      new_credits := (SELECT credits FROM profiles WHERE id = v_user_id);
      is_active := false;
      RETURN NEXT;
    END IF;
  END LOOP;
...
BEGIN
  ...
  SELECT EXISTS(
    SELECT 1 FROM tower_game_sessions
